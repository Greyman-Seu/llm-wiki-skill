...
## Karpathy llm-wiki 对齐原则

llm-wiki 必须保持三层结构：

| 层 | 含义 | 维护规则 |
|---|---|---|
| Raw sources | 用户提供的论文、PDF、网页、公众号、笔记、项目页、视频等原始材料 | 原始事实来源，AI 只读不改 |
| Wiki | `wiki/sources/`、`wiki/topics/`、`wiki/synthesis/`、`index.md`、`log.md` | AI 生成并持续维护 |
| Schema | `.wiki-schema.md`、`SKILL.md`、templates、校验脚本 | 约束 AI 如何维护 wiki |

FollowHub 形态下，前台主概念只保留三层：

```text
材料 -> 主题路线 -> 综述判断
```

- **raw source**：不可变原始输入，比如 PDF、arXiv、公众号原文、项目页。
- **材料 / source page**：AI 读完 raw source 后生成的阅读入口，也叫 source summary。
- **主题路线 / topic**：把多篇材料组织到同一条问题线。
- **综述判断 / synthesis**：跨材料、跨主题形成阶段性判断，回答“这些材料放在一起说明什么”。

`domain`、`source_type`、`tag` 只作为 metadata 和筛选字段；`entity`、`query`、`comparison` 只作为可选内部能力，不作为前台主导航。Graph 是 wiki 的视图，不维护第二套知识模型。R2 是发布产物，不改变 raw sources -> wiki -> schema 的源头逻辑。
...
| "发布 wiki"、"同步 R2"、"publish wiki"、"发布知识库"、"R2" | → **publish** |
...
2. 如果两者都没有：
   ...
   - `query` / `lint` / `status` / `digest` / `graph` / `publish` / `delete` → 提示用户先初始化知识库
3. 读取知识库根目录下的 `.wiki-schema.md`
4. 如果存在 `.wiki-vocabulary.md`，读取其中的 domain/tag 词表；ingest 时必须优先使用这份词表
5. 从 `.wiki-schema.md` 的"语言"字段判断 `WIKI_LANG`
   - `语言：中文` → `WIKI_LANG=zh`
...
**术语对照**：
- 原始材料 / 原始来源 → Raw Source
- 材料页 / 材料 → Source Page / Material
- 主题路线 / 主题 → Topic Route / Topic
- 综述判断 / 综述 → Synthesis Judgment / Synthesis
- 领域 → Domain
- 标签 → Tag
- 消化 → Ingest
- 结晶化 → Crystallize
...
**概念边界**：
- `raw source` 是用户提供的原始事实来源，AI 不改写。
- `source page` 是 AI 生成的材料阅读入口，存放在 `wiki/sources/`。
- `topic` 是多份材料共同指向的问题路线，不是普通标签。
- `synthesis` 是阶段性判断，不是材料摘要合集。
- `entity`、`query`、`comparison` 可以存在，但默认不作为前台主概念。
...
5. **补充初始化结果说明**：
   - `init-wiki.sh` 会同时生成 `purpose.md`、`.wiki-vocabulary.md` 和 `.wiki-cache.json`
   - `purpose.md` 和 `.wiki-schema.md` 同级存放，用来记录研究目标、关键问题和研究范围
   - `.wiki-vocabulary.md` 用来约束 domain/tag，避免标签越生成越散
...
3. **读取上下文**：
   - 优先顺序：`purpose.md` > `.wiki-schema.md` > `.wiki-vocabulary.md` > `index.md`
   - 如果 `purpose.md` 存在，先读取其中的核心目标、关键问题和研究范围
   - 如果 `.wiki-vocabulary.md` 存在，读取 domain/tag 词表；新材料分类必须优先复用已有 domain/tag
...
5. **Step 1：FollowHub 决策分析**：
   - 输入：原始内容 + `purpose.md` + `.wiki-schema.md` + 现有 wiki 结构（至少读取 `index.md`、相关 `wiki/sources/`、`wiki/topics/`、`wiki/synthesis/`）
   - 输出：JSON 格式的决策对象，不持久化，只在当前 ingest 流程里临时传递
   - 优先判断这次对 `source/topic/synthesis` 的影响，不要先创建一堆 entity
   - JSON 必须包含 `source_decision`、`classification`、`topic_decisions`、`synthesis_decision`、`graph_edges`、`warnings`
   ...
   ```json
   {
     ...
     "source_decision": {
       ...
       "target_slug": "",
       "reason": "",
       "dedupe_signals": ["url", "arxiv_id", "doi", "title", "pdf", "project", "github", "hjfy", "authors_date"]
     },
     "classification": {
       "material_type": "paper | wechat | blog | note | project | video | report | dataset | code",
       "source_type": "",
       "domains": ["LLM/VLM"],
     ...
     "graph_edges": [
       {
         "from": "",
         "to": "",
         "type": "supports | evidence_for | updates | contrasts | relates",
   ...
   决策规则：
   - 材料去重优先级：URL / arXiv ID / DOI / PDF / 项目页 / GitHub / hjfy / 标题相似度 / 作者日期。
   - 每个材料必须分配至少一个 domain，tag 控制在 1-2 个，最好 1 个。
   - topic 优先复用已有路线；只有形成新的持续问题线时才 `create_topic`。
   - synthesis 不机械更新；只有新材料改变阶段性判断、反驳已有 claim、显著补强证据、连接多个 topic，或回答 open question 时才 `update` / `create`。
   - 如果 synthesis 保持不变，也必须在 `reason` 中说明原因。
...
6. **Step 2：页面生成**：
   ...
   - **上下文加载规则**：只读取 Step 1 中 `source_decision.target_slug`、`topic_decisions[].target_slug`、`synthesis_decision.target_slug` 指向的已有页面；如果某页超过 2000 字，只读取 frontmatter + 需要更新的章节
   - 输出：所有需要创建或更新的 wiki 页面内容
   - Step 2 负责完成 source page、topic page、synthesis page、index、log 和 graph 关系更新

7. **容错回退**：
   - 如果 Step 1 不是有效 JSON，或者缺少 `source_decision`、`classification`、`topic_decisions`、`synthesis_decision` 等必需字段，自动回退到原来的单步流程
   - 回退时，所有本次新生成内容统一加上：
     ```markdown
...
9. **更新或创建主题路线**（`wiki/topics/{topic-slug}.md`）：
   - 对每个 `topic_decisions` 执行 attach 或 create
   - 如果已有 topic → 更新关键材料表、当前理解、未解决问题和相关综述
   - 如果新建 topic → 必须说明为什么它不是普通 tag，而是一条新的问题线
   - 使用 `[[主题名]]` 和 frontmatter `source_slugs` 同时维护人读链接和机器可解析关系

10. **按门槛更新综述判断**（`wiki/synthesis/{synthesis-slug}.md`）：
   - 如果 `synthesis_decision.action=unchanged`，不要改 synthesis，只在 ingest 结果里说明理由
   - 如果 `update`，只改受影响的 claim、证据、open question 或当前判断
   - 如果 `create`，参考 `templates/synthesis-template.md` 创建新的阶段性判断
   - synthesis 必须能追溯到 source/topic 证据

10b. **可选更新实体页**（`wiki/entities/`）：
   - 只有当一个概念被多篇材料反复使用、并且创建实体页能减少明显重复时，才创建或更新 entity
   - entity 不作为前台主入口，不应抢 source/topic/synthesis 的维护优先级
...
13. **发布前变更预览与结果展示**（按 `WIKI_LANG` 切换语言）：
...
2. **Step 0：调用脚本做机械检查**（必须先做，不要跳过）：
   ...
   同时运行 FollowHub schema 检查：

   ```bash
   bash ${SKILL_DIR}/scripts/validate-followhub-wiki.sh <wiki_root>
   ```

   `lint-runner.sh` 负责三项**机械检查**（只需要精确匹配，不需要判断）：
   - 孤立页面（`entities/` 下没有被其他页面引用的实体）
   - 断链（`[[X]]` 链接指向的 `X.md` 不存在，支持 `[[X|别名]]` 语法）
   - index 一致性（index.md 里有记录但文件缺失的条目）

   `validate-followhub-wiki.sh` 负责 FollowHub 三层 schema 检查：
   - source 是否有 `slug`、`type: source`、`material_type`、`domains`、`tags`、`raw_refs`
   - topic 是否有 `slug`、`type: topic`、`source_slugs`
   - synthesis 是否有 `slug`、`type: synthesis`、`judgment`、`source_slugs`、`topic_slugs`
...
**区别于 query**：query 是快速问答；digest 是跨材料、跨主题形成可沉淀的 synthesis judgment。

digest 不是把多篇材料重新摘要一遍，而是回答：
...
- 默认综述判断格式：`"给我讲讲 XX"、"深度分析 XX"、"综述 XX"、"digest XX"、"全面总结一下 XX"`
...
3. **生成结构化综述判断**，保存到 `wiki/synthesis/{主题}-{格式}.md`（按 `WIKI_LANG` 切换语言）：

   > 文件名规则：默认格式用 `{主题}-综述判断.md`，对比表用 `{主题}-对比.md`，时间线用 `{主题}-时间线.md`
   ...
   **zh**：
   ...
   ## 判断依据
   （按 claim 组织证据，每个证据标注 source/topic）
   - Claim 一（来源：[[素材A]]、[[主题A]]）
   - Claim 二（来源：[[素材C]]）
...
**FollowHub 规则**：graph 是 source/topic/synthesis 的视图，不是第二套知识模型。节点和边必须从 wiki 页面、frontmatter、`related_topics`、`related_syntheses`、`source_slugs`、`topic_slugs`、`synthesis_slugs` 派生。不要在 graph 里创造前台没有对应页面的主要概念。
...
2b. **生成交互式图谱数据**（`wiki/graph-data.json`）：
   ...
   脚本会优先扫描 `wiki/{sources,topics,synthesis}/*.md`，
   解析同行 `[[双向链接]]` 与 `<!-- confidence: EXTRACTED|INFERRED|AMBIGUOUS -->` 注释，
   调用本地 Node helper 计算关系权重（共引强度 / 来源重叠 / 类型亲和度）和规则 insights，
   并写入 `wiki/graph-data.json`（内容 >2MB 自动降级，单节点只留 500 行；图规模超预算时 insights 自动降级）。
   依赖 `python3` + `node`（macOS 通常自带 `python3`；如缺失可运行 `brew install python node`）。

2c. **图谱运行时说明**：
   - 图谱基础构建现在依赖 `python3` + `node`
   - 不需要额外 `npm install`
   - `node` 只用于运行随仓库分发的本地预构建 helper

2d. **生成交互式图谱 HTML**（FollowHub wiki 页面适配版）：
   ...
   页面只表达材料、主题路线、综述判断三层；节点点击应聚焦节点并给出对应 wiki route。
   不输出“社区”“推荐起点”“路线图”等额外前台概念。

3. **读取 insights 并向用户展示结果**（按 `WIKI_LANG` 切换语言）：

   先读取 insights：
   ```bash
   python3 -c 'import json,sys; print(json.dumps(json.load(open(sys.argv[1], encoding="utf-8")).get("insights", {}), ensure_ascii=False, indent=2))' "$WIKI_ROOT/wiki/graph-data.json"
   ```
   ...
   **zh**：
   ...
   图谱洞察：
   - 惊人连接：{from} ↔ {to}（跨层级或跨主题关联，权重 {weight}）{如有}
   - 桥节点：{node}（连接多条材料/主题/综述关系）{如有}
...
## 工作流 8b：publish（生成并发布 R2 数据包）

### 触发关键词

"发布 wiki"、"同步 R2"、"publish wiki"、"发布知识库"、"生成 R2 数据包"

**FollowHub 规则**：R2 是运行时读取层，不是知识源头。发布前必须先让本地 wiki 通过校验；发布产物必须能被前台作为唯一 wiki 数据源读取。
...
1. **校验本地 wiki**：
   ```bash
   bash ${SKILL_DIR}/scripts/validate-followhub-wiki.sh "<wiki 根目录>"
   ```

2. **生成图谱数据和 HTML**：
   ```bash
   bash ${SKILL_DIR}/scripts/build-graph-data.sh "<wiki 根目录>"
   bash ${SKILL_DIR}/scripts/build-graph-html.sh "<wiki 根目录>"
   ```

3. **生成 R2 数据包**：
   ```bash
   python3 ${SKILL_DIR}/scripts/build-followhub-wiki-package.py "<wiki 根目录>" "<package_dir>" --pretty
   ```
...
5. **发布**：
   ...
   bash ${SKILL_DIR}/scripts/publish-followhub-wiki-r2.sh "<package_dir>" rclone:<remote>:<bucket-or-prefix>/wiki
   ```

   `local:` 用于本地验收；`rclone:` 用于真实 R2。脚本发布前会再次校验 package。

6. **前台同步验证**（在 `page_github` 仓库）：
   ```bash
   node scripts/sync-wiki-data.mjs --package-dir "<package_dir>"
   node scripts/sync-wiki-data.mjs --base-url "$PUBLIC_WIKI_DATA_BASE_URL"
   ```

   线上构建使用 `sync:wiki:optional`，优先从 `PUBLIC_WIKI_DATA_BASE_URL` 同步；R2 暂不可用时保留已有 generated fallback。
...
材料：{source_count}
主题路线：{topic_count}
综述判断：{synthesis_count}
图谱关系：{edge_count}

发布目标：{target}
前台同步：{synced | skipped with fallback}
```
