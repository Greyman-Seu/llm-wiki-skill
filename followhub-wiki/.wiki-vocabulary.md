...
- 2026-08-10 tactile-representation：现有 seed tag 没有专门覆盖触觉表示学习。vision-language-action 偏向策略端、robot-foundation-model 太宽、Tactile Genesis 的 tag 用的是 tactile-sensing（更偏向传感本身）。tactile-representation 专门聚合"如何学可迁移触觉 backbone"的材料，已覆盖 2606.22332 (Tactile Genesis) + 2606.29948 (HTT)，未来触觉 MAE、跨模态触觉对齐类工作都可挂入。

- 2026-08-28 contact-rich-manipulation：现有 tactile-representation 关注传感与表示、vision-language-action 关注基础策略，均未覆盖运动—力联合控制、方向性柔顺与接触安全这条独立问题线。该 tag 由 2410.09309 (Adaptive Compliance Policy) 建立，未来可聚合擦拭、翻转、插入、沿面跟随和其他柔顺控制工作。
