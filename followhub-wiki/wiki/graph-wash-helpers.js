...
(function (root) {
  ...
  function normalizeAtlasNode(rawNode, index) {
    ...
    return {
      ...
      publish_date: raw.publish_date || raw.date || "",
  ...
  function resolveAtlasVisibleSnapshot(model, layout, uiState) {
    ...
    var scopedNodeId = safeUI.scopedNodeId == null ? null : String(safeUI.scopedNodeId);
    var filters = safeUI.filters && typeof safeUI.filters === "object" ? safeUI.filters : {};

    var scopedIds = null;
    if (scopedNodeId && safeModel.byId && safeModel.byId[scopedNodeId]) {
      scopedIds = {};
      scopedIds[scopedNodeId] = true;
      safeModel.edges.forEach(function (edge) {
        if (edge.source === scopedNodeId) scopedIds[edge.target] = true;
        if (edge.target === scopedNodeId) scopedIds[edge.source] = true;
      });
    }

    var baseNodes = scopedIds
      ? safeModel.nodes.filter(function (node) {
          return !!scopedIds[node.id];
        })
      : safeModel.nodes.filter(function (node) {
          if (activeCommunityId !== "all" && node.community !== activeCommunityId) return false;
          if (focusMode === "source" && node.type !== "source") return false;
          if (focusMode === "topic" && node.type !== "topic") return false;
          if (focusMode === "synthesis" && node.type !== "synthesis") return false;
          return true;
        });
